...
if __name__ == '__main__':
    ...
    # 僅支援固定路徑：data/data_to_train.pkl
    data_pkl_path = os.path.join("data", "data_to_train.pkl")
    assert os.path.exists(data_pkl_path), f"data file not found: {data_pkl_path}"
    with open(data_pkl_path, "rb") as h:
        data_to_train = pickle.load(h)
    dataset_dict = data_to_train["dataset_dict"]
    all_code_weights = data_to_train.get("all_code_weights", None)
    ...
    # ********************* Embedding ****************************************************************
    # 直接使用 featureSet 內的 concept_map_rev，並改為隨機初始化 embedding（不依賴預訓練檔案）
    all_concept_map_reverse = featureSet.concept_map_rev
    embedding_dim = 128  # size of embedding, must be multiple of number of heads --128
    window_days = 90  # kept for compatibility; not used when pre_trained=False
    ...
    model_params = {
        ...
        'use_mask': True, 'concept_embedding_path': None, 'normalize_visits': False,
        'normalize_codes': False, 'use_RNN': False, 'time_emb_type': 'sin', 'use_mask_later': True,
        'all_concept_map_reverse': all_concept_map_reverse, 'pre_trained': False,
    ...
    base_model.set_data(
        ...
        dataset_dict['births'], dataset_dict['genders'], all_code_weights=all_code_weights)
    base_model.cuda()
    ...
    for epoch in range(n_epochs_pretrain):
        ...
        for batch_num, (X, m, mask, label, duration, is_observed) in enumerate(train_loader):
            mask = torch.tensor(np.vstack(mask).astype(float)).cuda()
            label = torch.tensor(np.vstack(label).astype(float)).cuda()
            surv_pred, y_pred = clf(X.cuda(), m=None, train=True)
            surv_probs = torch.cumprod(surv_pred, dim=1)
            try:
        ...
        with torch.no_grad():
            ...
            # Save best model by validation C-index
            if val_cindex is not None and val_cindex > best_val_cindex:
                best_val_cindex = float(val_cindex)
                best_model_path = os.path.join(save_dir, "STRAFE_best.pth")
                torch.save({
                    'model_state_dict': clf.state_dict(),
                    'model_params': model_params,
                    'survival_arch': sap,
                    'embedding_dim': embedding_dim,
                    'concept_map': featureSet.concept_map,
                    'concept_map_rev': featureSet.concept_map_rev,
                    'max_time': max_time,
                }, best_model_path)
                print(f"[Checkpoint] Saved best model to: {best_model_path} (val_cindex={best_val_cindex:.4f})")
